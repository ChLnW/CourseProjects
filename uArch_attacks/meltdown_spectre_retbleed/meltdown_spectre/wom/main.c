...
static ssize_t device_read(struct file *file, char __user *buf, size_t len,
    ...
    /* FIXME: This doesn't look very much like write-only.
     * Could this be exploited by h-h-h-hackers? /me trembles */
    for (i = 0; i < len; ++i) {
        READ_ONCE(*(device_mem + pos + i));
    }

    return len;
}

static ssize_t device_write(struct file *file, const char __user *buf, size_t len,
                            loff_t *ppos) {
    loff_t pos = *ppos;
    int ret;
